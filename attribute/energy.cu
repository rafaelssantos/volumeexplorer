...
__global__ void _energyKernel(const Volume* volume, float *data, int texSide) {
    ...
    if(index < volume->voxelCount()) {
        ...
        _calcEnergy(histogram, nVoxels, buffer);
...
thrust::host_vector<float>* Energy::kernelLaunch(const Volume* volume, size_t threadCount, dim3 blockDim, int mask) {
    ...
    _energyKernel<<<gridDim, blockDim>>>(volume, thrust::raw_pointer_cast(&devVector[0]), mask);
    getLastCudaError("Energy kernel failed.");
